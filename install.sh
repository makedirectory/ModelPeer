...
USAGE
  ...
  # Each reviewer owns one output file. A `tee` per reviewer would put two model
  # responses on the same stdout and leave the reviewer's own exit status behind a
  # PIPESTATUS the parent cannot read once the pipeline is no longer in the
  # foreground — so the review is captured first and replayed afterwards.
  #
  ...
  for p in "${valid[@]}"; do
    ...
    if run_provider "$p" "$max_depth" "$prompt" "$timeout" > "$tmpdir/$p.txt"; then
      rc=0
    else
      rc=$?
    fi
