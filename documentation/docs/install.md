...
```bash
curl -fsSL https://raw.githubusercontent.com/makedirectory/ModelPeer/v0.6.1/install.sh | bash
```

Interactive setup, which can offer to install missing CLIs:

```bash
curl -fsSL https://raw.githubusercontent.com/makedirectory/ModelPeer/v0.6.1/install.sh | bash -s -- --setup
```
